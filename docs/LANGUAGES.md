...
TokToken supports 49 programming languages. Symbol extraction uses [universal-ctags](https://ctags.io/) for most languages and built-in custom parsers for 16 additional ones.
...
| Blade | `.blade.php` | 3-pass parser: dead zones (`{{-- comment --}}`, `@verbatim`), 19 `@directives` (extends, section, include variants, component, slot, livewire, push/prepend, inject, each, use), `<x-component>` and `<x-slot:name>` extraction. Import-aware (Blade-specific extractor) |
...
| Razor (ASP.NET) | `.cshtml` | C# methods from `@functions`/`@code` blocks, HTML element IDs, `@model`/`@using`/`@inject` directives |
| Twig | `.twig` | Blocks, macros (as methods), set variables, template references (`extends`/`include`/`embed`/`import`/`from`/`use`), HTML element IDs, Stimulus.js `data-controller` attributes. Handles `{# comment #}`, `{% verbatim %}`, `{% raw %}` dead zones and `[list, syntax]` template fallbacks |
...
- **Template languages**: only the 16 custom parsers above are supported. Other template engines (Jinja2, Handlebars, etc.) are stripped to their embedded language before ctags processing.
